#!/bin/bash
#SBATCH -J VTn_BFA_parse  #job name for array
#SBATCH -n 1                    # Number of cores
#SBATCH -N 1                    # Ensure that all cores are on one machine
#SBATCH -t 0-08:00              # Runtime in D-HH:MM
#SBATCH -p serial_requeue       # Partition to submit to
#SBATCH --mem=20000               # Memory pool for all cores (see also --mem-per-cpu)
#SBATCH -o ../../shell_outputs/VTn_BFA_%a.out      # File to which STDOUT will be written
#SBATCH -e ../../shell_outputs/VTn_BFA_%a.err      # File to which STDERR will be written
